...
public class ConfigService {
  ...
  private String toXml(TplInformativaPSP element) {
    try {
      JAXBElement<TplInformativaPSP> informativaPSP = new it.gov.pagopa.microservice.template.ObjectFactory().createInformativaPSP(
          element);
  ...
  private String toXml(CtListaInformativePSP element) {
    try {
      JAXBElement<CtListaInformativePSP> informativaPSP = new it.gov.pagopa.microservice.catalogodati.ObjectFactory().createListaInformativePSP(
          element);
      JAXBContext jc = JAXBContext.newInstance(element.getClass());
      Marshaller marshaller = jc.createMarshaller();
      marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
  ...
  private String toXml(String name, CtListaInformativeControparte element) {
    try {
      JAXBElement<CtListaInformativeControparte> informativaPA = new it.gov.pagopa.microservice.controparti.ObjectFactory().createListaInformativeControparte(
          element);
      JAXBContext jc = JAXBContext.newInstance(element.getClass());
      Marshaller marshaller = jc.createMarshaller();
      marshaller.setProperty(Marshaller.JAXB_ENCODING, "UTF-8");
